...
class GlobalBlock:
    def __init__(self,logfile,obsAlign,obsPars,obsFits):
...
class FitsFile:
 ...
 # Main list to accommodate multiple sci extensions, each element is a dict
    ...
    #         errtxt += ' x and y rotation angles disagree by > 1 deg: '+str(angle_x)+' '+str(angle_y)
      ...
      if self.PathName in defShiftDict.keys():
        ...
        if AlignParam.superalign != None:
          if self._use_simplematch_for_sol(refMatchFile, hdr_dx,\
            ...
            errtxt = "ERROR: Superalign failure."
            GlobalBlock.logfile.write(errtxt)
            raise Exception, errtxt
...
#              I = I + 1
	...
	""" Returns a tuple of final shifts uncertainties for an image.
	    ...
	    warnline =  'WARNING: using WCS information for %s shifts!'%(imname)
	    ...
	    self.logfile.write(warnline)
            ...
            self.MatchDict['angle']     = self.MatchDict['fit_angle']
	...
	shifts  = (self.PathName, xpix_shift, dxpix_shift, ypix_shift, dypix_shift,\
                   self.MatchDict['angle'],self.MatchDict['angle_err'])
	return shifts
    ...
    def getHeaderShift(self,refim):
	   ...
	   if self.simpleimage == 0:
           ...
           dx_pix /= refim.NumSci
           dy_pix /= refim.NumSci
           self.preMatchDict[ii]['hdr_xshift'] = -dx_pix
...
class AlignParam:
    def __init__(self,superalign,tolmatch,default_scale_tol,maxfitrms):
      # self.MatchParamList = []
...
class StandAlignParam(AlignParam):
    def __init__(self,superalign):
      AlignParam.__init__(self,superalign,tolmatch=6/20.0,default_scale_tol=0.003,maxfitrms=2.0/20.0)
...
class Align:
    ...
    def match(self,AlignParam,notrot,intRef = None,retry=0):
	...
	self.GlobalBlock.outputList['Match.shifts'] = []
        ...
        if not self.refImage:
            self.refImage = self._chooseRefimage(intRef=intRef)
    ...
    def _chooseRefimage(self,intRef=None):
      """Returns a reference image file name, based on Sxtractor trimmed results.
          ...
          for fitsfile in self.imfiles:
            if fitsfile.Asn == Asn:
              if (not self.refImage.extref) or (self.refImage.PathName != fitsfile.PathName):
                ...
                asnShiftFile.write('%s %9.4f %7.4f %9.4f %7.4f %12.6f %9.6f\n' %
                                   shifts)
...
class NIC3Camera(Camera):
    def __init__(self,pardir,modName):
      Camera.SearchParam = NIC3DefSearchParam(pardir,modName)
      TKWList = KWList()
      TKWList.AddKW("INSTRUME","NICMOS")
      TKWList.AddKW("FOCUS","CAMERA3")
      Camera.__init__(self,"NIC3",TKWList,0,0.20)
      del TKWList
      TKWList = KWList()
      TKWList.AddKW("FILTER","F110W")
      Camera.AddFilter(self,Filter('F110W','/data2/bouwens/testnic/nic3_idc.fits',TKWList,NIC3DefSearchParam(pardir,modName)))
      del TKWList
      TKWList = KWList()
      TKWList.AddKW("FILTER","F160W")
      Camera.AddFilter(self,Filter('F160W','/data2/bouwens/testnic/nic3_idc.fits',TKWList,NIC3DefSearchParam(pardir,modName)))
      del TKWList
