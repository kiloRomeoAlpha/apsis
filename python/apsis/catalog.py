...
class Catalog: 
    ...
    The richField argument determines which parameter set will be use for WFC
    ...
    def __init__(self, obs, richField=None, excludeList=None): 
        ...
        if obs.detector == 'WFC':
            if richField:
                self.defaultInParFile = os.path.join(obs.pardir,self.modName+'_wfc_richfield.inpar')
                self.logfile.write("Selected rich field WFC input parameter set.")
            else:
                self.defaultInParFile = os.path.join(obs.pardir,self.modName+'_wfc_sparsefield.inpar') 
                self.logfile.write("Selected WFC sparse field input parameter set for SExtractor.")
        elif obs.detector == 'HRC':
            if richField:
                self.defaultInParFile = os.path.join(obs.pardir,self.modName+'_hrc_richfield.inpar')
                self.logfile.write("Selected rich field HRC input parameter set.")
            else:
                self.defaultInParFile = os.path.join(obs.pardir,self.modName+'_hrc_sparsefield.inpar') 
                self.logfile.write("Selected sparse field HRC input parameter set for SExtractor.")
        else:
            self.defaultInParFile = os.path.join(obs.pardir,self.modName+'.inpar')
            self.logfile.write("Detector is not WFC,HRC. Using default input parameter set.")
    ...
    def setpars(self,fitsfile):
        ...
        paramFileName   = os.path.join(self.obsPars, fitsname + '.param')
        ...
        self.setinpar('FLAG_IMAGE'     , os.path.join(self.obsFits,   flag   )) 
        ...
        for im in checkImageTypes:
            imFileName = fitsname+'_'+im[:4]+'.fits' 
